/* http://prismjs.com/download.html?themes=prism&languages=markup+css+clike+javascript+aspnet+bash+ruby+java+php+python */
          ...
          for (var s = 0; s < o.length; ++s) {
            ...
            for (var p = 0; p < r.length; p++) {
              ...
              if (!(m instanceof a)) {
                ...
                if (y) {
                  g && (f = y[1].length);
                  var w = y.index + f, y = y[0].slice(f), _ = w + y.length, S = m.slice(0, w), O = m.slice(_),
                    j = [p, v];
                  S && j.push(S);
...
!function (e) {
  var t = {
    ...
    }, {pattern: /\$\([^)]+\)|`[^`]+`/, inside: {variable: /^\$\(|^`|\)$|`$/}}, /\$(?:[a-z0-9_#\?\*!@]+|\{[^}]+\})/i]
  };
  e.languages.bash = {
